...
exports.parse = function (post, cb) {
  ...
  const tagname_re = /^\/?(?:b|i|u|pre|center|samp|code|colou?r|size|noparse|url|link|s|q|(block)?quote|img|u?list|li)$/i
  ...
  const newline_tags = ['pre', 'code', 'center', 'img', 'li', 'quote']
  ...
  if ( post ) {
    ...
    result = post
      ...
      .replace(/<\/([a-z]{1,16})><br>/g, (mstr, m1) => {
        return newline_tags.includes(m1) ? `</${m1}>` : mstr
      })
  }
