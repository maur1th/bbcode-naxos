...
exports.parse = function (post, cb) {
  ...
  function textToHtmlCB(mstr, m1, m2, m3, m4, offset, string)
  {
    ...
    //
    // Ignore tags which will not close
    //
    ...
    if ( isValidTag(m2) ) {
      ...
      if ( opentags.length && (opentags[opentags.length-1].bbtag == "url" ||
          opentags[opentags.length-1].bbtag == "link") && urlstart >= 0) {
        return "[" + m2 + "]"
      }
  ...
  if ( post ) {
    ...
    post = (function (_post) {
      return _post.replace(/(\[\*\])([^\[]*)/g, function(m0, m1, m2) {
        return '[li]' + m2 + '[/li]'
      })
    })(post)
    ...
    if ( opentags.length ) {
      ...
      if (opentags[opentags.length-1].bbtag == "url" ||
          opentags[opentags.length-1].bbtag == "link") {
        opentags.pop()
